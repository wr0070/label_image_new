...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def edit_label(self):
        ...
        # 补齐或裁剪为 3 个
        texts = texts + [""] * (3 - len(texts))
        texts = texts[:3]

        # 传入弹窗
        new_text = self.label_dialog.pop_up(texts)

        if new_text is not None:
            item.setText(new_text)
            item.setBackground(generate_color_by_text(new_text))
            self.set_dirty()
    ...
    def shape_selection_changed(self, selected=False):
        if self._no_selection_slot:
            ...
            if shape:
                # 🔥 修复关键：shape 可能不在字典里，先判断
                if shape not in self.shapes_to_items:
                    return    # 不进行任何后续动作，避免 KeyError & 避免重复标签
    ...
    def load_labels(self, shapes):
        """
        shapes: list of tuples (label, points, line_color, fill_color, difficult)
        """
        s = []

        # 清空画布已有标签，避免重复
        self.canvas.shapes.clear()
        self.shapes_to_items.clear()  # 同步清空映射
        self.label_hist.clear()       # 可选：清空历史标签

        for label, points, line_color, fill_color, difficult in shapes:
            shape = Shape(label=label)
            shape.paint_label = True  # 一定显示文字
    ...
    # Callback functions:
    def new_shape(self):
        if not self.single_class_mode.isChecked():
            self.prev_label_text = ""
        self.lastLabel = ""
        ...
        if not self.use_default_label_checkbox.isChecked():
            if self.single_class_mode.isChecked() and self.lastLabel:
                ...
                # 弹窗，传入上一次标签，避免重复
                text = self.label_dialog.pop_up(texts=[self.prev_label_text] if self.prev_label_text else None)
                self.lastLabel = text
        else:
